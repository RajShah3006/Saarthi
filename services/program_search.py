...
class ScoreBreakdown:
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "prestige": round(self.prestige, 3),
...
class ProgramSearchService:
    ...
    PROGRAM_SPECIFIC_RANKINGS: Dict[str, Dict[str, float]] = {
        "computer science": {
            "university of waterloo": 1.0,      # #1 for CS in Canada
            "university of toronto": 0.95,
            "university of british columbia": 0.90,
            "mcgill university": 0.85,
            "university of alberta": 0.75,
            # ... others default to general ranking
        },
        "engineering": {
            "university of waterloo": 1.0,
            "university of toronto": 0.95,
            "mcgill university": 0.90,
            "queen's university": 0.85,
            "mcmaster university": 0.80,
        },
        "business": {
            "western university": 1.0,          # Ivey
            "queen's university": 0.95,         # Smith
            "university of toronto": 0.90,      # Rotman
            "york university": 0.85,            # Schulich
            "mcgill university": 0.85,          # Desautels
        },
        "health sciences": {
            "mcmaster university": 1.0,
            "university of toronto": 0.95,
            "queen's university": 0.85,
            "western university": 0.80,
        },
    }
    ...
    def _calculate_prestige_score(self, program: Program, user_fields: List[str]) -> float:
        """Get prestige score, considering program-specific rankings"""
        uni_name = (program.university_name or "").lower().strip()
        
        # Check program-specific rankings first
        for field in user_fields:
            if field in self.PROGRAM_SPECIFIC_RANKINGS:
                field_rankings = self.PROGRAM_SPECIFIC_RANKINGS[field]
                for uni, score in field_rankings.items():
                    if uni in uni_name or uni_name in uni:
                        return score
        
        # Fall back to general rankings
        return self.PROGRAM_SPECIFIC_RANKINGS.get(uni_name, 0.50)
    ...
    def _calculate_final_score(
        ...
        prestige_score = self._calculate_prestige_score(program, user_fields)
        breakdown.prestige = prestige_score
        ...
        final = (
            ...
            weights.prestige * breakdown.prestige
        )
