...
class RoadmapService:
    ...
    def _program_to_payload(self, prog: Program, score: float, bd: Dict[str, Any]) -> Dict[str, Any]:
        """Convert Program + breakdown into a compact dict for UI embedding (no embeddings)."""
    ...
    def _format_output(self, profile: StudentProfile, results: List[Tuple], ai_response: str) -> str:
        ...
        prefs_line = f"**Preferences:** {prefs}  " if prefs else ""
...
"""
        ...
        for i, (prog, score, bd) in enumerate(results, 1):
            if bd["final"] >= 0.7:
                match_icon, match_text = "🌟", "Excellent Match"
            elif bd["final"] >= 0.5:
                match_icon, match_text = "✅", "Good Match"
            elif bd["final"] >= 0.3:
                match_icon, match_text = "🟡", "Moderate Match"
            else:
                match_icon, match_text = "⚪", "Low Match"
