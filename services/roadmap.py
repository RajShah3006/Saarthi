...
class RoadmapService:
    ...
    def _format_full_plan_ai(
        ...
        system = (
            ...
            "- Do NOT invent deadlines beyond provided timeline_events.\n"
            "- Do NOT change program requirements; keep as provided.\n"
            "- Output ONLY Markdown.\n"
            "- Make it readable: short sections, bullet points, a small table.\n"
        )

        payload_json = json.dumps(payload, ensure_ascii=False, default=_json_default)

        prompt = (
            "Format this data into a polished student-friendly plan with these sections:\n"
            "1) Profile\n"
            "2) Top Programs (table: Program | University | Match | Co-op | Prereqs)\n"
            "3) Timeline (dated bullets)\n"
            "4) Checklist Projects (checkbox style)\n"
            "5) Personalized Analysis (cleaned)\n\n"
            "DATA (JSON):\n"
            f"{payload_json}"
        )
